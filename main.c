...
void processt1(FILE* fp_out,TreeNode* pTree){
    ...
    if(!isEmpty(pTree)){
        fprintf(fp_out, "%d;%f", pTree->ID,pTree->temp);
    }
}
...
TreeNode* insertABRt1(TreeNode* pTree, int ID, float temp){
    FILE *fp_in = fopen("data_temp.csv", "r");
    if (fp_in == NULL) {
        exit(14);
    }
    if(isEmpty(pTree)){
...
int sortMethod(TreeNode* pTree, int ID, float temp){
        int A = 0;
        int method = 0;
      ...
      if(A==1){
        switch (method) {
            case 1: // Tri par ABR
                puts("OKOKOK ABR");
                insertABRt1(pTree, ID, temp);
                break;
            case 2: // Tri par AVL
                // Code pour le tri par AVL
            default:
                // avl
            break;
      ...
      if(strcmp(input_filet1, "sorttemperature.csv")==0){
          ...
          if(strcmp(output_filet1, "sorttemperature.txt") == 0) {
              ...
              if(strcmp(mode, "-t1") == 0) {
                  ...
                  if(strcmp(option,"--abr") == 0) {
                      fp_in = fopen(input_filet1, "r");
                      if (fp_in == NULL) {
                          return ERROR_INPUT_FILE;
                      }
                      while (Check_file(fp_in) == 0) {
                          fscanf(fp_in, "%d;%f", &ID, &temp);
                          insertABRt1(pTree, ID, temp);// mode pour le tri par ABR
                      }
                      fp_out = fopen(output_filet1, "w");
                      if (fp_out == NULL) {
                          return ERROR_OUTPUT_FILE;
                      }
                      Traversal_Inft1(fp_out, pTree);//parcours infixe
                      fclose(fp_in);
                      fclose(fp_out);
                  }
                      ...
                      while (Check_file(fp_in) == 0) {
                            fscanf(fp_in, "%d;%f", &ID, &temp);
                            // insertAVLt1(pTree, ID, temp);// mode pour le tri par AVL
                            // prog avl ...
                      }
                      fp_out = fopen(output_filet1, "w");
                      if (fp_out == NULL) {
          ...
          if(strcmp(output_filep1, "sortpressure.txt")==0) {
              ...
              if(strcmp(mode, "-p1") == 0) {
                  ...
                  if(strcmp(option,"--abr") == 0) {
                      fp_in = fopen(input_filep1, "r");
                      if (fp_in == NULL) {
                          return ERROR_INPUT_FILE;
                      }
                      while (Check_file(fp_in) == 0) {
                          fscanf(fp_in, "%d;%f", &ID, &press);
                          insertABRp1(pTree, ID, press);// mode pour le tri par ABR
                      }
                      fp_out = fopen(output_filet1, "w");
                      if (fp_out == NULL) {
                          return ERROR_OUTPUT_FILE;
                      }
                      Traversal_Infp1(fp_out, pTree);//parcours infixe
                      fclose(fp_in);
                      fclose(fp_out);
                  }
                  else if (strcmp(option, "--avl") == 0) {
              ...
              if(strcmp(mode, "-h") == 0) {
                  ...
                  if(strcmp(option,"--abr") == 0) {
                      ...
                      while (Check_file(fp_in) == 0) {
                          ...
                          insertABRh(pTree, ID, height);// mode pour le tri par ABR
                      }
                      fp_out = fopen(output_fileh, "w");
                      if (fp_out == NULL) {
                          return ERROR_OUTPUT_FILE;
                      }
                      Traversal_Infh(fp_out, pTree);//parcours infixe
                      fclose(fp_in);
                      fclose(fp_out);
                  }
                  else if (strcmp(option, "--avl") == 0) {
              ...
              if(strcmp(mode, "-m") == 0) {
                  ...
                  if(strcmp(option,"--abr") == 0) {
                      ...
                      while (Check_file(fp_in) == 0) {
                          ...
                          insertABRm(pTree, ID, moisture);// mode pour le tri par ABR
                      }
                      fp_out = fopen(output_filem, "w");
                      if (fp_out == NULL) {
                          return ERROR_OUTPUT_FILE;
                      }
                      Traversal_Infm(fp_out, pTree);//parcours infixe
                      fclose(fp_in);
                      fclose(fp_out);
                  }
                  else if (strcmp(option, "--avl") == 0) {
